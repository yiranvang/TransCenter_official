...
from datasets.transcenter_dataset.coco import COCO
...
def main(args):
    ...
    if args.resume:
        ...
        if "r50_deformable_detr-checkpoint.pth" in args.resume:
            missing_keys = []
            unexpected_keys = []
            model_dict = model_without_ddp.state_dict()
            for k, v in checkpoint['model'].items():
                if k in model_dict.keys():
                    if "backbone" in k or "encoder" in k or "input_proj" in k:
                        model_dict[k] = v
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint and \
            "r50_deformable_detr-checkpoint.pth" not in args.resume:
            import copy
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm, adaptive_clip=args.adaptive_clip)
        lr_scheduler.step()
