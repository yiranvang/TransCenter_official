...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        self.curr_pre = nn.Linear(d_model, d_model)
        self.curr_pre_dropout = nn.Dropout(dropout)
        self.curr_pre_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, pre_srcs=None, pre_masks=None, pre_hms=None,
                pre_pos_embeds=None):
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def split_featsV2(self, spatial_shapes, level_start_index, hidden, pre_hidden):
        ...
        for lvl in range(len(spatial_shapes)):
            ...
            split_hidden.append(
                hidden[:, start_indx:end_indx, :].view(b, curr_shape[0], curr_shape[1], c).permute(0, 3, 1,
                                                                                                   2).contiguous())

            pre_split_hidden.append(
                pre_hidden[:, start_indx:end_indx, :].view(b, curr_shape[0], curr_shape[1], c).permute(0, 3, 1,
                                                                                                       2).contiguous())
    ...
    def forward(self, tgt, pre_tgt, reference_points, src, src_spatial_shapes, src_level_start_index,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            split, pre_split = self.split_featsV2(src_spatial_shapes, src_level_start_index, output,
                                                  pre_output)
